.macro lerInteiro($descitor)
    ...
    while_number:
        ...
        lb $t2, ($a1)
        beq $t2, '\r', while_number # CRLF -> ignora e lê o \n
        beq $t2, '\n', fim_number
...
main:
    ...
    li $s1, 0
    while_elementos:
        lerInteiro($s0)          # Lê um inteiro do arquivo
        blez $v0, fim_elementos  # Chegou ao fim do arquivo
        addi	$s1, $s1, 1	     # $s1 = $s1 + 1
        j while_elementos        # jump para while_elementos
    fim_elementos:
    ...
    li $a1, 0        # modo somente leitura
    syscall
    beqz $v0, invalido # Checa se foi possível abrir o arquivo (se $v0 == 0)
    move $s0, $v0    # s0 = descritor
    ...
    move $a0, $t0 # $t0 bytes to be allocated
    li $v0, 9		 # comando alocar memoria
    syscall					
    ...
    while_numeros:
        ...
        j while_numeros # jump para while_numeros
    fim_numeros:
...
encontrarMaior: # ($a0 = vetor, $a1 = tamanho)
    lw	$t1, 0($a0)		# int maior = vetor[0];
    li $t0, 1  # i(t0) = 1
    for_maior: # for(i = 1; i < $a1; i++)
    ...
    c:
        addi $t0, $t0, 1 # i++
        j for_maior		 # jump para for_maior
    fim_maior:
    move $v0, $t1		 # $v0 = $t1
    jr $ra				 # jump to $ra

  
encontrarMenor: # ($a0 = vetor, $a1 = tamanho)
    lw	$t1, 0($a0)		# int maior = vetor[0]
    li $t0, 1 # i(t0) = 1
    for_menor: # for(i = 1; i < $a1; i++)
    ...
    c1:
        addi $t0, $t0, 1 # i++
        j for_menor		 # jump para for_menor
    fim_menor:
    move $v0, $t1		 # $v0 = $t1
    jr $ra

    
contarImpares: # ($a0 = vetor, $a1 = tamanho)
    li $t1, 0		# $t1 = 0
    li $t0, 0 # i(t0) = 0
    for_impar: # for(i = 0; i < $a1; i++)
    ...
    nao_impar:
        addi $t0, $t0, 1 # i++
        j for_impar		 # jump para for_impar
    fim_impar:
    move $v0, $t1		 # $v0 = $t1
        jr $ra

   
contarPares: # ($a0 = vetor, $a1 = tamanho)
    li $t1, 0		# $t1 = 0
    li $t0, 0 # i(t0) = 0
    for_par: # for(i = 0; i < $a1; i++)
...
ordenarCrescente: # ($a0 = vetor, $a1 = tamanho)
    ...
    li $t0, 0 # i(t0) = 0
    for_crescente1: # for(i = 0; i < tamanho - 1; i++)
        bge $t0, $t2, fim_crescente1 # Se i >= $a1 então vai para fim
        addi $t1, $t0, 1	# j = i + 1
        for_crescente2: # for(j = i + 1; j < $a1; j++)
            bge $t1, $a1, fim_crescente2 # Se j >= $a1 então vai para fim
                ...
                fim_if_crescente:
            addi $t1, $t1, 1 # j++
            j for_crescente2		# jump para for_crescente2
        fim_crescente2:
        addi $t0, $t0, 1 # i++
        j for_crescente1	# jump para for_crescente1
    fim_crescente1:
    jr $ra
 ...
 ordenarDecrescente: # ($a0 = vetor, $a1 = tamanho)
    subi $t2, $a1, 1	# $t2 = $a1 - 1
    li $t0, 0 # i(t0) = 0
    for_decrescente1: # for(i = 0; i < tamanho - 1; i++)
        bge $t0, $t2, fim_decrescente1 # Se i >= $a1 então vai para fim
        addi $t1, $t0, 1			# j = i + 1
        for_decrescente2: # for(j = i + 1; j < $a1; j++)
            bge $t1, $a1, fim_decrescente2 # Se j >= $a1 então vai para fim
                ...
                fim_if_decrescente:
            addi $t1, $t1, 1 # j++
            j for_decrescente2		# jump para for_decrescente2
        fim_decrescente2:
        addi $t0, $t0, 1 # i++
        j for_decrescente1	# jump para for_decrescente1
    fim_decrescente1:
    jr	$ra


calcularProduto: # ($a0 = vetor, $a1 = tamanho)
    li	$v0, 1		# $v0 = 1
    li $t0, 0 # i(t0) = 0
    for_produto: # for(i = 0; i < $a1; i++)
        ...
        addi $t0, $t0, 1  # i++
        j for_produto	  # jump para for_produto
    fim_produto:
    jr $ra
...
imprimirVetor: # ($a0 = vetor, $a1 = tamanho)
    ...
    for_vetor: # for(i = 0; i < $tamanho; i++)
        ...
        addi $t0, $t0, 1 # i++
        j for_vetor		 # jump para for_vetor
    fim_vetor:
    jr $ra